...
class Admin extends CI_Controller {   
    ...
    public function video_upload(){
     ...
     public function episodes_upload(){
        $response['status'] = 'error';
        $response['errors'] = "You don't have permission to upload video";
        if ($this->session->userdata('admin_is_login') == 1):
            ...
            if(!empty($_POST["videos_id"]))
                $videos_id              = $_POST["videos_id"];
            if(!empty($_POST["seasons_id"]))
                $seasons_id             = $_POST["seasons_id"];
            if(!empty($_POST["episodes_name"]))
            ...
            if(isset($_FILES["FileInput"]) && $_FILES["FileInput"]["error"]== UPLOAD_ERR_OK)
                {
                    ...
                    if(move_uploaded_file($_FILES['FileInput']['tmp_name'], $UploadDirectory.$NewFileName ))
                    {
                        ...
                        ini_set('memory_limit ', '1256MB');
                        if(isset($_FILES["thumbnail"]) && $_FILES["thumbnail"]["error"]== UPLOAD_ERR_OK):
                            move_uploaded_file($_FILES['thumbnail']['tmp_name'], 'uploads/episodes/'.$episodes_id.'.jpg');
                            $image_url = base_url('uploads/episodes/'.$episodes_id.'.jpg');
                        endif;
                        $response['status'] = 'success';
                        $response['video_info'] = '<tr id="row_'.$episodes_id.'"><td><img src="'.$image_url.'" width="80"></td><td><strong>'.$episodes_name.'</strong><td><strong>Upload</strong></td><td>'.base_url().'uploads/videos/'.$NewFileName.'</a></td><td><a href="#" onclick="javascript:location.reload();">Reload Page</a></td></tr>';
                    }else{
                        $response['status'] = 'error';
                        $response['errors'] = 'error uploading File!';
                    }                
                ...
                {
                    $response['status'] = 'error';
                    $response['errors'] = 'Something wrong with upload! Is.Please contact with hosting providet to increase upload file size,execution time and post max size.';
                }
            endif;
        echo json_encode($response);
    }
